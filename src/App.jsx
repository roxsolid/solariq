...
const QUIZ=[
  ...
  {id:"goal",q:"What matters most to you?",hint:"This shapes the whole recommendation",opts:[
    {label:"Survive power outage", Icon:Ico.Battery, v:"bk",kw:3},
  ...
  {id:"ls",q:"How bad is power outage?",hint:"Determines your battery backup size",opts:[
...
const ERRORS={"F01":{brand:"Sunsynk",title:"Grid voltage too high",sev:"warning",diy:true,fix:"Grid voltage above safe range — usually Eskom. Resolves itself. If it persists over 2 hours, contact your installer.",specs:["Inverter Repair"]},"F02":{brand:"Sunsynk",title:"Grid voltage too low",sev:"warning",diy:true,fix:"Grid voltage dropping below safe threshold. Common during power outage transitions. System auto-switches to battery.",specs:["Inverter Repair"]},"F32":{brand:"Sunsynk",title:"Battery over-temperature",sev:"critical",diy:false,fix:"Battery overheating. Ensure ventilation immediately. Do NOT continue using — contact technician urgently.",specs:["Battery Replacement","Full System Service"]},"E001":{brand:"Victron",title:"Low battery shutdown",sev:"warning",diy:true,fix:"Battery depleted to minimum safe level. Will resume charging once power is available.",specs:["Battery Replacement"]},"E002":{brand:"Victron",title:"Overload — too much drawn",sev:"warning",diy:true,fix:"Drawing more power than inverter can handle. Switch off heavy appliances and restart.",specs:["Inverter Repair"]},"E003":{brand:"Victron",title:"Inverter overheating",sev:"critical",diy:false,fix:"Switch off immediately. Ensure 20cm clearance on all sides. Do not restart until cool.",specs:["Inverter Repair","Full System Service"]},"W001":{brand:"Deye",title:"PV input voltage high",sev:"info",diy:true,fix:"Panel voltage slightly above optimal. Usually resolves as panels cool. Monitor for 24 hours.",specs:["Full System Service"]},"W003":{brand:"Deye",title:"Grid frequency out of range",sev:"warning",diy:true,fix:"Eskom frequency unstable. Normal during power outage transitions.",specs:["Inverter Repair"]},"G01":{brand:"Growatt",title:"No grid connection detected",sev:"info",diy:true,fix:"Check your mains breaker first. If mains is on and not power outage, contact your installer.",specs:["Inverter Repair"]},"G05":{brand:"Growatt",title:"Insulation resistance fault",sev:"critical",diy:false,fix:"Serious fault. Switch off at DC isolator immediately. Call a qualified electrician now.",specs:["Full System Service","Inverter Repair"]}};
...
const ARTICLES=[
  ...
  {id:5,tag:"Guide",hot:false,min:"8",views:"3.9k",title:"Off-grid vs grid-tied solar in South Africa — the honest truth",intro:"The dream of zero electricity bill is real — but not for everyone.",body:[{h:"Grid-tied: cheapest, useless in power outage",p:"No battery, no backup. System switches off during power outage. Only makes sense if you're never affected."},{h:"Hybrid: the SA sweet spot",p:"Grid plus battery. Handles power outage, reduces bill. What 95% of SA residential installations should be. Cost: R80,000–R200,000."},{h:"Off-grid: freedom, but expensive",p:"Needs 3× the battery capacity of hybrid. Makes sense for farms — not most SA suburbs."},{h:"Recommendation",p:"For urban SA: go hybrid. Size battery for 2× your power outage hours with 20% buffer."}],related:[1,2,6]},
...
function Results({r,onReset,goInstallers}){
  ...
  const bullets=(
    <div style={{background:t.bgCard,border:`1px solid ${t.border}`,borderRadius:13,padding:"16px",marginBottom:12}}>
      {[`Lights, WiFi, TV & fridge through all power outage`,`Save ~R${Math.round(r.annSave/12).toLocaleString()} every month`,`Pays for itself in ${r.payback} years — then free electricity`,`Claim up to R15,000 back from SARS on panel costs`,`Property value increases R50k–R150k`].map(txt=>(
          ...
          <div key={p.id} onClick={()=>setActive(p)} style={{background:t.bgCard,border:`1px solid ${t.border}`,borderRadius:14,overflow:"hidden",cursor:"pointer",transition:"all .2s",animation:`fadeUp .35s ease ${i*.07}s both`}}
            ...
            <div style={{padding:"14px"}}>
              <div style={{display:"flex",gap:6,marginBottom:8,alignItems:"center"}}>
                <Tag>{p.tag}</Tag>{p.hot&&<span style={{fontSize:14}}>🔥</span>}
...
export default function App(){
  ...
  const sc=useScreen();
  const t=DARK; // dark only, light mode removed
          ...
          <nav style={{background:t.navBg,backdropFilter:"blur(20px)",borderBottom:`1px solid ${t.border}`,padding:"0 28px",position:"sticky",top:0,zIndex:200,flexShrink:0}}>
            <div style={{maxWidth:1360,margin:"0 auto",display:"flex",alignItems:"center",justifyContent:"space-between",height:52,width:"100%"}}>
              ...
              <div style={{display:"flex",gap:2}}>
                {NAV.map(x=><button key={x.id} onClick={()=>goTab(x.id)} onMouseEnter={e=>{if(tab!==x.id){e.currentTarget.style.color=t.textMid;e.currentTarget.style.boxShadow=`0 0 12px rgba(${t.rgb},.15)`;e.currentTarget.style.background=`rgba(${t.rgb},.04)`;e.currentTarget.style.borderColor=`rgba(${t.rgb},.15)`;} }} onMouseLeave={e=>{if(tab!==x.id){e.currentTarget.style.color=t.sub;e.currentTarget.style.boxShadow="none";e.currentTarget.style.background="none";e.currentTarget.style.borderColor="transparent";} }} style={{background:tab===x.id?`rgba(${t.rgb},.08)`:"none",border:`1px solid ${tab===x.id?`rgba(${t.rgb},.22)`:"transparent"}`,color:tab===x.id?t.accent:t.sub,padding:"5px 16px",borderRadius:7,cursor:"pointer",fontSize:12,fontWeight:600,transition:"all .18s",fontFamily:B}}>{x.l}</button>)}
              </div>
            ...
            {tab==="home"&&(
              <div style={{animation:"fadeUp .5s ease"}}>
                ...
                <div style={{background:`linear-gradient(135deg,rgba(${t.rgb},.08),rgba(${t.rgb},.03))`,border:`1px solid rgba(${t.rgb},.15)`,borderRadius:16,padding:sc.isMobile?"20px":"32px",textAlign:"center"}}>
                  ...
                  <p style={{color:t.sub,fontSize:14,marginBottom:18,lineHeight:1.7,maxWidth:400,margin:"0 auto 18px"}}>Weekly deals, maintenance tips and power outage updates. No spam, unsubscribe anytime.</p>
