...
const ARTICLES=[
  ...
  {id:7,tag:"News",hot:true,min:"5",views:"3.1k",title:"Sodium-ion batteries are coming to SA — and they could change everything",intro:"A new battery technology is making its way to South Africa. Cheaper than lithium, no cobalt, doesn't catch fire.",coverImg:"https://images.unsplash.com/photo-1593941707882-a5bba14938c7?w=800&q=80",photos:[{url:"https://images.unsplash.com/photo-1518770660439-4636190af475?w=600&q=80",caption:"Sodium-ion cell construction differs fundamentally from lithium"},{url:"https://images.unsplash.com/photo-1581091226825-a6a2a5aee158?w=600&q=80",caption:"Manufacturing scale-up is driving costs down rapidly"}],body:[{h:"What is sodium-ion?",p:"Sodium-ion batteries work on the same principle as lithium-ion — ions move between electrodes to store and release energy. The key difference is the charge carrier: sodium instead of lithium. Sodium is the 6th most abundant element on Earth. Lithium is scarce and concentrated in a handful of countries."},{h:"Why it matters for South Africa",p:"SA's solar market has boomed since 2022 load shedding hit Stage 6. Demand for batteries has pushed prices up and created supply shortages. Sodium-ion batteries could break this bottleneck — they can be manufactured anywhere, without the rare minerals that make lithium expensive."},{h:"The specs that matter",p:"Current Na-ion cells hit 140–160 Wh/kg energy density — roughly 70–80% of entry-level LFP lithium. That means a slightly larger battery pack for the same storage. But the tradeoff: safer chemistry, longer cycle life in extreme temperatures, and projected costs 20–30% below lithium by 2027."},{h:"Who's bringing it to market",p:"CATL — the world's largest battery manufacturer — announced mass production of Na-ion cells in 2023. BYD has a parallel programme. Both supply Deye, Sunsynk and other brands sold in SA. First Na-ion home storage products are expected locally by late 2026."},{h:"Should you wait?",p:"No. If you need solar now, install lithium LFP — it's proven, warrantied and available. Na-ion is 12–18 months away from hitting SA shelves at competitive prices. Think of it as a future option for your next upgrade cycle, not a reason to delay."}],related:[1,2,6]},
  {id:8,tag:"Review",hot:false,min:"11",views:"2.4k",title:"Pylontech US3000C review — is it still the best home battery for SA in 2026?",intro:"The Pylontech US3000C has been SA's most popular home battery for three years. We tested one in a real Johannesburg household for 60 days.",coverImg:"https://images.unsplash.com/photo-1620714223084-8fcacc2dbe6d?w=800&q=80",photos:[{url:"https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=600&q=80",caption:"The US3000C rack-mount design makes installation clean and expandable"},{url:"https://images.unsplash.com/photo-1473341304170-971dccb5ac1e?w=600&q=80",caption:"BMS data via the Pylontech app during our 60-day test period"}],youtubeId:"dQw4w9WgXcQ",youtubeTitle:"Pylontech US3000C — Full Installation & 60-Day Performance Review",affiliate:{url:"https://example.co.za/pylontech-us3000c",label:"Check current price at SolarAdvice →",disclosure:"SolarIQ earns a small commission if you purchase through this link. This does not affect our review — we tested this unit independently."},body:[{h:"Specs at a glance",p:"3.5kWh usable capacity per unit. 48V nominal. LFP chemistry. 6,000 cycle life rated to 80% capacity retention. Max 74A charge/discharge. CAN/RS485 comms. Stackable to 8 units (28kWh). Weight: 34kg."},{h:"Installation experience",p:"The US3000C comes with clear documentation and the BMS communications cable included. Pairing with a Sunsynk 5kW hybrid took about 40 minutes including rack mounting. The CAN communication link detected the battery automatically — no manual parameter setting required."},{h:"Real-world performance — 60 days",p:"Test home: 3-bedroom in Northcliff, Johannesburg. Average daily consumption: 22kWh. Single US3000C (3.5kWh usable). Load shedding: Stage 4 most of the test period. The battery handled a 2-hour Stage 4 slot comfortably running lights, WiFi, TV and a small fridge."},{h:"What we liked",p:"Build quality is exceptional — the rack feels industrial-grade. BMS communication is rock solid. After 200 charge cycles during the test, capacity showed zero measurable degradation. Pylontech's local warranty support is the best in the market."},{h:"What could be better",p:"The 3.5kWh per unit means you need two for most SA households — that's R28,000–R34,000 for two units before installation. The newer US5000 (4.8kWh) offers better value per kWh. The app is functional but dated."},{h:"Verdict",p:"Still one of the safest bets in the SA market. The US3000C earns its reputation through consistency, local support and bulletproof BMS communications. If budget allows, look at the US5000 for better kWh-per-rand. But if your installer quotes US3000C — don't hesitate."}],rating:{overall:4.4,value:4.0,build:5.0,software:3.5,support:4.5},related:[1,2,6]},
];

function useCount(t,ms=1300){const[v,setV]=useState(0);useEffect(()=>{let s=null;const f=ts=>{if(!s)s=ts;const p=Math.min((ts-s)/ms,1);setV(Math.floor((1-Math.pow(1-p,3))*t));if(p<1)requestAnimationFrame(f);};requestAnimationFrame(f);},[t]);return v;}
function makeResult(d,k,bf=1.5){const mo=Math.round(d*30*RATE),cost=Math.round(k*18000),save=Math.round(mo*12*.75);return{systemKw:k,battKwh:Math.round(k*bf*10)/10,cost,annSave:save,mo,payback:(cost/save).toFixed(1),dailyKwh:Math.round(d*10)/10,panels:Math.ceil(k/.55)};}
...
function ProCalc({onResult}){
  ...
  const Sec=({id,icon,label,children,cols})=>(
    <div style={{marginBottom:14}}>
      <button onClick={()=>toggle(id)} style={{width:"100%",background:"none",border:"none",display:"flex",alignItems:"center",gap:8,cursor:"pointer",padding:"6px 0",marginBottom:open[id]?10:0}}>
        ...
        <span style={{fontSize:13,color:t.sub,transition:"transform .25s",display:"inline-block",transform:open[id]?"rotate(90deg)":"rotate(0deg)"}}>›</span>
      </button>
      {open[id]&&<div style={{display:"grid",gridTemplateColumns:cols||"1fr",gap:8}}>{children}</div>}
    </div>
  );
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,marginBottom:14}}>
        {[["Panels",`${panels}×`,"550Wp"],["Array",`${syskw.toFixed(1)}kWp`,"Total"],["Battery",`${batKwh.toFixed(1)}kWh`,"Usable"],["Backup",`${backupH}h`,"Avg load"],["Inverter",`${v.invKva}kVA`,"Min"],["Cost",`R${(cost/1000).toFixed(0)}k`,"Installed"]].map(([l,val,s])=>(
          <div key={l} style={{textAlign:"center"}}>
            <div style={{fontFamily:H,fontSize:sc.isMobile?17:20,fontWeight:W.hero,color:t.accent}}>{val}</div>
  ...
  const inputs=(
    <div>
      <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}>
        ...
        <div>
          <div style={{fontFamily:H,fontSize:24,fontWeight:W.hero,color:t.text}}>Pro Calculator</div>
      ...
      <Sec id="load" icon="⚡" label="Load & Generation" cols={sc.isMobile?"1fr":"1fr 1fr"}>
      ...
      <Sec id="battery" icon="🔋" label="Battery Bank" cols={sc.isMobile?"1fr 1fr":"1fr 1fr 1fr"}>
      ...
      <Sec id="type" icon="🔌" label="System Type">
...
function Calculator({onResult}){
    ...
    <div style={{opacity:fade?0:1,transition:"opacity .2s",animation:"fadeUp .5s ease"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:sc.isDesktop?"repeat(4,1fr)":"1fr 1fr",gap:12}}>
        {[{k:"simple",icon:"✨",title:"Quick & Easy",sub:"4 questions. 60 seconds.",badge:"Most Popular"},{k:"appliance",icon:"🔌",title:"By Appliances",sub:"Pick every device you own.",badge:"Most Accurate"},{k:"bill",icon:"📄",title:"From My Bill",sub:"Enter your Eskom bill.",badge:"Fastest"},{k:"engineer",icon:"⚙️",title:"Pro Calculator",sub:"Full technical inputs.",badge:"Pro"}].map(c=>(
          <div key={c.k} onClick={()=>go(()=>setMode(c.k))} style={{background:t.bgCard,border:`1px solid ${t.border}`,borderRadius:16,padding:"20px 16px",cursor:"pointer",transition:"all .2s",position:"relative"}}
            ...
            <div style={{fontFamily:H,fontSize:18,fontWeight:W.card,color:t.text,marginBottom:4}}>{c.title}</div>
  ...
  if(mode==="bill")return(
    <div style={{opacity:fade?0:1,transition:"opacity .2s",animation:"fadeUp .4s ease",maxWidth:560,margin:"0 auto"}}>
  ...
  if(mode==="appliance")return(
    <div style={{opacity:fade?0:1,transition:"opacity .2s"}}>
      ...
      <div style={{display:"flex",borderBottom:`1px solid ${t.border}`,marginBottom:13,overflowX:"auto"}}>
        {["essentials","work","home","comfort"].map(c=><button key={c} onClick={()=>setCat(c)} style={{background:"none",border:"none",borderBottom:`2px solid ${cat===c?t.accent:"transparent"}`,color:cat===c?t.accent:t.sub,padding:"7px 12px",cursor:"pointer",fontSize:12,fontWeight:600,textTransform:"capitalize",transition:"all .2s",fontFamily:B,whiteSpace:"nowrap"}}>{c}</button>)}
      </div>
      <div style={{display:"grid",gridTemplateColumns:`repeat(auto-fill,minmax(${sc.isMobile?100:120}px,1fr))`,gap:8,marginBottom:20}}>
        {APPLIANCES.filter(a=>a.cat===cat).map(app=>{
          ...
          return(
            <div key={app.id} style={{background:active?`rgba(${t.rgb},.08)`:t.bgCard,border:`1px solid ${active?`rgba(${t.rgb},.4)`:t.border}`,borderRadius:12,padding:10,textAlign:"center",transition:"all .2s",cursor:"pointer"}}
              onClick={active?undefined:()=>{setApps({...apps,[app.id]:app.h});}}>
              <div style={{fontSize:20,marginBottom:3}} onClick={()=>!active&&setApps({...apps,[app.id]:app.h})}>{app.icon}</div>
              <div style={{fontSize:11,fontWeight:600,color:active?t.text:t.sub,marginBottom:1,fontFamily:B}}>{app.name}</div>
              ...
              {active&&(
                <div style={{marginTop:7}}>
                  ...
                  <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:4}}>
                    <button onClick={e=>{e.stopPropagation();setApps({...apps,[app.id]:Math.max(.25,hrs-.25)});}} style={{background:`rgba(${t.rgb},.1)`,border:"none",color:t.text,width:26,height:26,borderRadius:6,cursor:"pointer",fontSize:16,display:"flex",alignItems:"center",justifyContent:"center"}}>-</button>
                    <span style={{fontSize:12,fontWeight:800,color:t.accent,minWidth:22,textAlign:"center"}}>{hrs}</span>
                    <button onClick={e=>{e.stopPropagation();setApps({...apps,[app.id]:Math.min(24,hrs+.25)});}} style={{background:`rgba(${t.rgb},.1)`,border:"none",color:t.text,width:26,height:26,borderRadius:6,cursor:"pointer",fontSize:16,display:"flex",alignItems:"center",justifyContent:"center"}}>+</button>
                  </div>
                </div>
              )}
            </div>
  ...
  const q=QUIZ[step];
  return(
    <div style={{opacity:fade?0:1,transition:"opacity .2s",maxWidth:640,margin:"0 auto"}}>
      ...
      <div key={step} style={{animation:"fadeUp .3s ease"}}>
        <div style={{textAlign:"center",marginBottom:20}}>
          <h3 style={{fontFamily:H,fontSize:"clamp(18px,3vw,26px)",fontWeight:W.section,color:t.text,marginBottom:5}}>{q.q}</h3>
  ...
  const sidebar=(
    <div style={{position:"sticky",top:80}}>
      <div style={{background:t.bgCard,border:`1px solid ${t.border}`,borderRadius:16,padding:"20px",marginBottom:12}}>
        <div style={{fontFamily:H,fontSize:15,fontWeight:W.section,color:t.text,marginBottom:14}}>System overview</div>
        {[["Daily usage",`${r.dailyKwh} kWh/day`,"#4ade80"],["Battery backup","Covers full LS slot","#60a5fa"],["Annual output",`~${Math.round(r.dailyKwh*365*.85)} kWh`,"#c084fc"],["25-year output",`~${Math.round(r.dailyKwh*365*.85*25/1000)} MWh`,t.accent],["CO₂ saved",`~${Math.round(r.dailyKwh*365*.85*25*.9)}kg`,"#4ade80"]].map(([l,v,c])=>(
      ...
      <div style={{textAlign:"center",marginTop:10}}><button onClick={onReset} style={{background:"none",border:"none",color:t.sub,cursor:"pointer",fontSize:13,textDecoration:"underline",fontFamily:B}}>← Recalculate</button></div>
    </div>
  );
  return(
...
function Installers(){
  ...
  const listEl=(
    <div style={{display:"flex",flexDirection:"column",gap:9}}>
      ...
      <div style={{marginTop:9,background:`linear-gradient(135deg,rgba(${t.rgb},.08),rgba(${t.rgb},.03))`,border:`1px solid rgba(${t.rgb},.18)`,borderRadius:14,padding:"16px 18px"}}>
  ...
  return(
    <div>
      ...
      <div style={{display:"flex",gap:8,marginBottom:10,flexWrap:"wrap"}}>
        <div style={{flex:1,minWidth:sc.isMobile?"100%":180,position:"relative"}}>
...
function Servicing(){
  ...
  const TechCard=({tech,i})=>(
    <div style={{background:t.bgCard,border:`1px solid ${t.border}`,borderRadius:13,padding:"14px 16px",animation:`fadeUp .35s ease ${i*.07}s both`}}>
      <div style={{display:"flex",alignItems:"flex-start",gap:10,marginBottom:10}}>
        ...
        <div style={{fontFamily:H,fontSize:14,fontWeight:W.section,color:t.accent,flexShrink:0}}>{tech.price}</div>
      </div>
  ...
  if(page==="home")return(
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:sc.isDesktop?"repeat(4,1fr)":sc.isMobile?"1fr 1fr":"repeat(auto-fill,minmax(200px,1fr))",gap:10,marginBottom:14}}>
        {[{id:"health",icon:"🩺",title:"Health Check",desc:"6 questions to diagnose your system.",badge:"AI",color:"#4ade80"},{id:"error",icon:"⚠️",title:"Error Code Translator",desc:"Type any inverter code. Plain English instantly.",badge:"Instant",color:t.accent},{id:"techs",icon:"🔧",title:"Find a Technician",desc:"Verified repair specialists near you.",badge:null,color:"#60a5fa"},{id:"reminder",icon:"📅",title:"Service Reminders",desc:"WhatsApp reminders when service is due.",badge:"Free",color:"#c084fc"}].map((c,i)=>(
          <div key={c.id} onClick={()=>setPage(c.id)} style={{background:t.bgCard,border:`1px solid ${t.border}`,borderRadius:14,padding:sc.isMobile?"14px 12px":"18px",cursor:"pointer",transition:"all .22s",position:"relative",animation:`fadeUp .35s ease ${i*.07}s both`}}
            ...
            <div style={{fontFamily:H,fontSize:sc.isMobile?14:16,fontWeight:W.card,color:t.text,marginBottom:3}}>{c.title}</div>
        ...
        {sc.isDesktop?(
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:28,alignItems:"start"}}>
            ...
            <div style={{background:t.bgCard,border:`1px solid ${t.border}`,borderRadius:14,padding:"18px"}}>
              <div style={{fontFamily:H,fontSize:15,fontWeight:W.section,color:t.text,marginBottom:12}}>Common error codes</div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:28,alignItems:"start"}}>
            <div>
              <div style={{textAlign:"center",marginBottom:22}}>
                ...
                <p style={{color:t.sub,fontSize:14,lineHeight:1.7}}>{hResult.note}</p>
              </div>
            ...
            <div style={{background:t.bgCard,border:`1px solid ${t.border}`,borderRadius:14,padding:"18px"}}>
              <div style={{fontFamily:H,fontSize:15,fontWeight:W.section,color:t.text,marginBottom:14}}>Diagnostic Breakdown</div>
              {[["Panel Efficiency",hResult.score*.3+60,"#f5a623"],["Battery Health",hResult.score*.4+50,"#4ade80"],["Inverter Status",hResult.score*.5+45,"#60a5fa"]].map(([l,v,c])=>(
                <div key={l} style={{marginBottom:14}}>
                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:5}}><span style={{fontSize:13,color:t.sub}}>{l}</span><span style={{fontSize:13,fontWeight:700,color:c}}>{Math.min(100,Math.round(v))}%</span></div>
    ...
    return(
      <div style={{maxWidth:sc.isDesktop?560:undefined,margin:sc.isDesktop?"0 auto":undefined}}>
        ...
        <div key={hStep} style={{animation:"fadeUp .3s ease"}}>
          ...
          <h3 style={{fontFamily:H,fontSize:sc.isMobile?19:22,fontWeight:W.section,color:t.text,marginBottom:14,lineHeight:1.2}}>{q.q}</h3>
...
function ArticleView({article,onBack}){
  ...
  const body=(
    <div>
      ...
      <h1 style={{fontFamily:H,fontSize:"clamp(20px,3vw,34px)",fontWeight:W.hero,color:t.text,lineHeight:1.15,marginBottom:14}}>{article.title}</h1>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:20,marginBottom:24}}>
        {article.body.map((sec,i)=>(
          <div key={i}>
            <h2 style={{fontFamily:H,fontSize:sc.isMobile?16:19,fontWeight:W.section,color:t.text,marginBottom:8}}>{sec.h}</h2>
  ...
  const sidebar=(
    <div style={{position:"sticky",top:80}}>
      <div style={{background:`linear-gradient(135deg,rgba(${t.rgb},.1),rgba(${t.rgb},.04))`,border:`1px solid rgba(${t.rgb},.2)`,borderRadius:14,padding:"18px",textAlign:"center",marginBottom:12}}>
        <div style={{fontFamily:H,fontSize:16,fontWeight:W.section,color:t.text,marginBottom:4}}>Ready to Calculate Your System?</div>
        <p style={{color:t.sub,fontSize:13,marginBottom:13}}>Free calculator — personalised result in under 2 minutes.</p>
        <PBtn sm style={{width:"100%",padding:"10px 24px"}}>☀️ Calculate My System</PBtn>
      </div>
      {related.length>0&&(
        <div>
          <div style={{fontFamily:H,fontSize:15,fontWeight:W.section,color:t.text,marginBottom:10}}>Related Articles</div>
          {related.map(rel=>(
            <div key={rel.id} onClick={()=>onBack(rel)} style={{background:t.bgCard,border:`1px solid ${t.border}`,borderRadius:10,padding:"12px 14px",cursor:"pointer",transition:"all .2s",display:"flex",alignItems:"center",gap:10,marginBottom:7}}
      ...
      {sc.isDesktop?(
        ...
        <div>
          ...
          <div style={{background:`linear-gradient(135deg,rgba(${t.rgb},.1),rgba(${t.rgb},.04))`,border:`1px solid rgba(${t.rgb},.2)`,borderRadius:14,padding:"18px",textAlign:"center",marginBottom:20}}>
            <div style={{fontFamily:H,fontSize:16,fontWeight:W.section,color:t.text,marginBottom:4}}>Ready to Calculate?</div>
...
function Blog(){
  ...
  return(
    <div>
      ...
      <div onClick={()=>setActive(list[0])} style={{background:`linear-gradient(135deg,rgba(${t.rgb},.08),rgba(${t.rgb},.03))`,border:`1px solid rgba(${t.rgb},.15)`,borderRadius:15,overflow:"hidden",marginBottom:12,cursor:"pointer",transition:"all .2s"}}
        ...
        {list[0].coverImg&&<img src={list[0].coverImg} alt="" style={{width:"100%",height:sc.isMobile?140:220,objectFit:"cover",display:"block"}}/>}
        <div style={{padding:"18px 20px"}}>
          ...
          <h3 style={{fontFamily:H,fontSize:"clamp(14px,2.5vw,20px)",fontWeight:W.section,color:t.text,marginBottom:7,lineHeight:1.2}}>{list[0].title}</h3>
      ...
      <div style={{display:"grid",gridTemplateColumns:sc.isMobile?"1fr":sc.isDesktop?"repeat(3,1fr)":"1fr 1fr",gap:10}}>
        {list.slice(1).map((p,i)=>(
          <div key={p.id} onClick={()=>setActive(p)} style={{background:t.bgCard,border:`1px solid ${t.border}`,borderRadius:13,overflow:"hidden",cursor:"pointer",transition:"all .2s",animation:`fadeUp .35s ease ${i*.07}s both`}}
            ...
            <div style={{padding:"12px 14px"}}>
              ...
              <h4 style={{fontFamily:H,fontSize:13,fontWeight:W.card,color:t.text,lineHeight:1.3,marginBottom:8}}>{p.title}</h4>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
...
function ComingSoon(){
  ...
  const[particles]=useState(()=>Array.from({length:22},(_,i)=>({id:i,x:Math.random()*100,y:Math.random()*100,size:Math.random()*3+1,dur:Math.random()*8+6,delay:Math.random()*5,opacity:Math.random()*.5+.1})));
  useEffect(()=>{
  ...
  return(
    <div style={{position:"fixed",inset:0,background:"#06080c",zIndex:9999,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"24px 20px",overflow:"hidden"}}>
      ...
      <div style={{position:"absolute",inset:0,pointerEvents:"none"}}>
        {particles.map(p=>(
          <div key={p.id} style={{position:"absolute",left:`${p.x}%`,top:`${p.y}%`,width:p.size,height:p.size,borderRadius:"50%",background:"#f5a623",opacity:p.opacity,animation:`particle ${p.dur}s ease-in-out ${p.delay}s infinite`}}/>
        ))}
      </div>
      ...
      <div style={{position:"relative",zIndex:1,display:"flex",flexDirection:"column",alignItems:"center",width:"100%",maxWidth:520}}>
        ...
        <div style={{textAlign:"center",marginBottom:32,animation:"fadeUp .7s ease"}}>
          <h1 style={{fontFamily:"'Lexend',sans-serif",fontSize:"clamp(24px,6vw,46px)",fontWeight:900,color:"#f0f0f0",lineHeight:1.1,marginBottom:12}}>
      ...
      <div style={{fontFamily:B,background:t.bg,minHeight:"100vh",color:t.text,transition:"background .35s,color .35s",overflowX:"hidden",display:"flex",flexDirection:"column"}}>
        ...
        {!sc.isMobile&&(
          <nav style={{background:t.navBg,backdropFilter:"blur(20px)",borderBottom:`1px solid ${t.border}`,padding:"0 24px",position:"sticky",top:0,zIndex:200,flexShrink:0}}>
            <div style={{maxWidth:1280,margin:"0 auto",display:"flex",alignItems:"center",justifyContent:"space-between",height:52,width:"100%"}}>
        ...
        <div style={{flex:1,width:"100%"}}>
          <div style={{maxWidth:1280,margin:"0 auto",padding:sc.isMobile?"16px 14px 100px":"40px 40px 60px",width:"100%"}}>
              ...
              <div style={{animation:"fadeUp .5s ease"}}>
                <div style={{display:"grid",gridTemplateColumns:sc.isDesktop?"1fr 1fr":"1fr",gap:sc.isDesktop?56:32,alignItems:"center",marginBottom:sc.isMobile?32:52}}>
                  <div>
                    ...
                    <h1 style={{fontFamily:H,fontSize:"clamp(34px,4vw,56px)",fontWeight:W.hero,lineHeight:1.05,marginBottom:16,color:t.text}}>
                ...
                <div style={{marginBottom:28}}>
                  <div style={{textAlign:"center",marginBottom:18}}>
                    ...
                    <h2 style={{fontFamily:H,fontSize:sc.isMobile?19:24,fontWeight:W.section,color:t.text}}>SolarIQ is with you at every stage</h2>
                  </div>
                ...
                <div style={{background:`linear-gradient(135deg,rgba(${t.rgb},.08),rgba(${t.rgb},.03))`,border:`1px solid rgba(${t.rgb},.15)`,borderRadius:16,padding:sc.isMobile?"20px":"28px",textAlign:"center"}}>
                  ...
                  <h3 style={{fontFamily:H,fontSize:sc.isMobile?18:22,fontWeight:W.section,color:t.text,marginBottom:5}}>Solar insights for SA homeowners</h3>
